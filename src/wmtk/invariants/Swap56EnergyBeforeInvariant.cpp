...
Swap56EnergyBeforeInvariant<AttributeType>::Swap56EnergyBeforeInvariant(
    const Mesh& m,
...
template <typename AttributeType>
bool Swap56EnergyBeforeInvariant<AttributeType>::before(const simplex::Simplex& t) const
{
    ...
    std::array<Eigen::Vector3<AttributeType>, 7> positions = {
    ...
    if constexpr (std::is_same<AttributeType, Rational>()) {
        positions_double = {
            {positions[0].template cast<double>(),
             ...
             positions[6].template cast<double>()}};
    } else {
...
template class Swap56EnergyBeforeInvariant<Rational>;
template class Swap56EnergyBeforeInvariant<double>;
