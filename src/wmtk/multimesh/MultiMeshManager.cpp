...
bool MultiMeshManager::simplex_is_in_child(
    const Mesh& my_mesh,
    const Mesh& child_mesh,
    const simplex::Simplex& my_simplex) const
{
    const ChildData& child_data = m_children[child_mesh.m_multi_mesh_manager.child_id()];
    ...
    auto tuples = simplex::cofaces_single_dimension_iterable(
        my_mesh,
        my_simplex,
        child_mesh.top_simplex_type());
    ...
    */
    ...
    for (const Tuple& tuple : tuples) {
        if (!map_tuple_between_meshes(my_mesh, child_mesh, map_accessor, tuple).is_null()) {
            return true;
        }
    }
