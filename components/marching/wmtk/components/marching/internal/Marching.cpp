...
Marching::Marching(
    ...
    , m_inversion_invariant(std::make_unique<SimplexInversionInvariant<double>>(
          pos_handle.mesh(),
...
void Marching::process()
{
    ...
    // test for inversions
    if (!is_inversion_free(m_mesh.get_all(m_mesh.top_simplex_type()))) {
        log_and_throw_error("Mesh is not inversion free BEFORE marching!");
    }
    ...
    // test for inversions
    if (!is_inversion_free(m_mesh.get_all(m_mesh.top_simplex_type()))) {
        log_and_throw_error("Mesh is not inversion free after marching.");
    }
    ...
    // clean up
    {
        ...
        keeps.emplace_back(m_pos_handle);
        keeps.emplace_back(m_vertex_tag_handle);
        if (m_edge_tag_handle.has_value()) {
            keeps.emplace_back(m_edge_tag_handle.value());
        }
        if (m_face_tag_handle.has_value()) {
            keeps.emplace_back(m_face_tag_handle.value());
        }
...
bool Marching::is_inversion_free(const std::vector<Tuple>& tets) const
{
    return m_inversion_invariant->after({}, tets);
}
