...
public:
    ...
    /*
     * @brief Check if all given top simplices are inversion free
     */
    bool is_inversion_free(const std::vector<Tuple>& tets) const;
...
private:
    ...
    std::unique_ptr<SimplexInversionInvariant<double>> m_inversion_invariant;
};
