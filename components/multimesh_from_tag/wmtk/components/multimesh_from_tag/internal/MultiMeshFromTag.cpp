    ...
    const PrimitiveType ptype)
{
    ...
    while (!q.empty()) {
        ...
        const simplex::IdSimplex cell = child.get_id_simplex(t, child.top_simplex_type());
        ...
        for (const Tuple& t_version : pt_intersection) {
            ...
            const simplex::IdSimplex child_face_0 =
                child.get_id_simplex(child_faces[0].tuple(), child.top_simplex_type());
            const simplex::IdSimplex child_face_1 =
                child.get_id_simplex(child_faces[1].tuple(), child.top_simplex_type());

            assert(child_face_0 == cell || child_face_1 == cell);

            q.push(child_face_0 == cell ? child_faces[1].tuple() : child_faces[0].tuple());
        }
    }
...
void MultiMeshFromTag::build_adjacency_matrix()
{
    ...
    for (const Tuple& cell_tuple : top_dimension_child_tuples) {
        ...
        const simplex::IdSimplex cell = child.get_id_simplex(cell_tuple, child.top_simplex_type());
        for (const Tuple& ft : face_tuples) {
            ...
            const Tuple neighbor =
                child_face_0 == cell ? child_faces[1].tuple() : child_faces[0].tuple();
